                        ...
                        <div class="other-schedule">
                            ...
                            <Popover :ref="(el) => setPopoverRef(el, lesson.код)">
                                <div class="other-schedule-buttons">
                                    <Button text @click="ScheduleGroup(lesson)">Расписание {{ lesson?.группа }}</Button>
...
const idAudLine = route.params.idAudLine;
const audName = ref("");
...
const headerTitles = ["Аудитория", "Учебный год", "Семестр", "Текущая неделя"];
const headerValues = computed(() => [
    scheduleData.value?.aud?.name,
...
watch(audName, (newAudName) => {
    localStorage.setItem('audName', newAudName);
    document.title = `${newAudName} - Расписание`;
});
...
const ScheduleGroup = (lesson) => {
    ...
    router.push(`/schedule/group/${lesson.кодГруппы}`)
}
...
const fetchAvailableDates = async () => {
    try {
        const response = await axios.get(`https://umu.sibadi.org/api/GetRaspDates?idAudLine=${idAudLine}`);
...
const fetchScheduleData = async () => {
    if (!idAudLine || !selectedDate.value) return;
    ...
    try {
        const response = await axios.get(`https://umu.sibadi.org/api/Rasp?idAudLine=${idAudLine}${sdateParam}`);
        scheduleData.value = response.data.data.info;
        
        audName.value = scheduleData.value?.aud?.name;
        saveScheduleSelection({
            type: SCHEDULE_TYPE_ROOM,
            id: idAudLine,
            name: scheduleData.value?.aud?.name,
            setAsLast: true
        });
